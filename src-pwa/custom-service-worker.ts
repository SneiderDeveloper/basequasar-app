...
const executeTransaction = (key: string, objectStore: string): Promise<any> => {
...
const updateCacheData = async (data, storageKey: string, key) => {
  const { request, storage } = await executeTransaction(storageKey, 'storage'); 
  const storedData = request.result?.data || [];
  const newData = storedData.map(item => {
    const match = String(item[key]) === String(data[key])
...
const getOfflineSetting = async (): Promise<boolean | null> => {
  try {
    ...
    const { request } = await executeTransaction(SITE_SETTINGS, OBJECT_STORE);
    const storedData = request.result?.data;
    ...
    while (entry = await queue.shiftRequest()) {
      try {
        ...
        if (method === 'PUT') {
          ...
          updateCacheData(data, KEY, 'id')
        }

        if (method === 'POST') {
          const { data } = await response.json();
          if (data?.offlineId) {
            updateCacheData(data, KEY, 'offlineId')
          }
